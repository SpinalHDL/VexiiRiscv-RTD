...
- Can run baremetal applications as well as Linux / Buildroot / Debian / ...
...
- **HART** : Hardware Thread. One CPU core can for instance implement multiple HART, meaning that it will execute multiple threads concurrently.
...
- **CSR** : Control Status Register, those are the special register in the CPU which allows to handle interruptions, exceptions as well as configuring things like the MMU.
...
- **MMU** : Memory Management Unit. Translate virtual addresses into physical ones, as well as check access permissions.
- **PMP** : Physical Memory Protection. Check physical address access permissions.
- **I$** : Instruction Cache
- **D$** : Data Cache
...
- **Interrupt** : An interrupt is a kind of trap which is generally coming from the outside. Ex :  timer, GPIO, UART, Ethernet, ...
...
- **Writeback** : The action which free a modified cache line by writing is back to the main memory
...
- **GShare** : This is a branch prediction technique which try to correlate branch instruction addresses,
...
- `RISC-V calling convention` : Mainly specify how the registers can be used by functions to pass parameters, as well as providing an alternative name for each of the registers (ex : x2 become the stack pointer, named sp)
...
Also due to the nested structure of the code base, a text editor / IDE which support curly brace folding can be very useful.
